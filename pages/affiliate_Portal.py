...
# NEW: rfid_fee added to the function parameters!
    ...
    for _, t in pending.iterrows():
        with st.expander(f"START TRIP: {t['make']} {t['model']} ({t['plate']}) - Renter: {t['renter_fullname']}"):
            ...
            if f"contract_{t['id']}" in st.session_state:
                ...
                st.markdown(f'<a href="data:application/pdf;base64,{st.session_state[f"contract_{t["id"]}"]}" download="Contract_DRV_{t["id"]:05d}.pdf" style="display:block; text-align:center; padding:15px; background-color:#28a745; color:white; font-size:18px; font-weight:bold; border-radius:8px; text-decoration:none; margin-bottom:10px;">📥 DOWNLOAD PDF CONTRACT</a>', unsafe_allow_html=True)
                    ...
                    if st.button("GENERATE CONTRACT & PREPARE DISPATCH", key=f"ex_{t['id']}", type="primary", use_container_width=True):
                        ...
                        if not (chk_tank and chk_ac and chk_wiper and chk_exterior and chk_seats and chk_deposit): st.error("NOTICE: Check off all items.")
                            ...
                            try:
                                ...
                                st.session_state[f"contract_{t['id']}"] = base64.b64encode(pdf_bytes).decode()
    ...
    for _, t in ongoing.iterrows():
        with st.expander(f"RECEIVE RETURN: {t['make']} {t['model']} ({t['plate']}) - Renter: {t['renter_fullname']}"):
            ...
            if f"ret_receipt_{t['id']}" in st.session_state:
                ...
                st.markdown(f'<a href="data:application/pdf;base64,{st.session_state[f"ret_receipt_{t["id"]}"]}" download="Settlement_DRV_{t["id"]:05d}.pdf" style="display:block; text-align:center; padding:15px; background-color:#28a745; color:white; font-size:18px; font-weight:bold; border-radius:8px; text-decoration:none; margin-bottom:10px;">📥 DOWNLOAD SETTLEMENT PDF</a>', unsafe_allow_html=True)
                ...
                if st.button("GENERATE SETTLEMENT & COMPLETE JOURNEY", key=f"comp_{t['id']}", type="primary", use_container_width=True):
                    ...
                    if not d_ok and not img_damage: st.error("NOTICE: You must upload a photo of the damage before settling!")
                        ...
                        try:
                            # Passed rfid_fee into the PDF function call
                            ...
                            st.session_state[f"ret_receipt_{t['id']}"] = base64.b64encode(pdf_bytes).decode()
